    ...
    let
      ...
      maybeConvert =
        ...
        if isEntry def.value then
          if elemType.check def.value then
...
in
{
  ...
  dagOf =
    elemType:
    let
      attrEquivalent = types.attrsOf (dagEntryOf false elemType);
    in
    mkOptionType rec {
      name = "dagOf";
  ...
  dalOf =
    elemType:
    let
      listEquivalent = types.listOf (dagEntryOf true elemType);
    in
    mkOptionType rec {
      name = "dalOf";
      ...
      substSubModules = m: dalOf (elemType.substSubModules m);
      functor = {
        name = name;
        type = dalOf;
        wrapped = elemType;
